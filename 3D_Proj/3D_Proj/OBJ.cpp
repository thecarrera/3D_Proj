...
void OBJ::LoadModel(std::string fileName, ID3D11Device* gDevice, ID3D11Buffer* &gVertexBuffer, ID3D11Buffer* &shaderBuffer)
{
	...
	while (ss.good())
	{
		...
		else if (strcmp(cmd, "f") == 0)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				(float)triangleArray.x	 = vertexPos.at(temp_vert[i].x - 1).x;
...
void OBJ::Texture(ID3D11Device* &gDevice, ID3D11ShaderResourceView* &gTextureRTV)
{
	HRESULT hr;
	...
	hr = DirectX::CreateWICTextureFromFile(gDevice, NULL, test, NULL, &gTextureRTV, NULL);
